	  ...
	  (function(d, s, id){
		...
		<!-- Header -->
			<header id="header" class="skel-layers-fixed">
				<h1><a href="index.html">Restaurants</h1>
				<nav id="nav">
					<ul>
		...
		<!-- Main -->
			<section id="main" class="container">
				...
				<div class="box">
					...
					<h3>Webpage on construction!</h3>
					<p></p>
					<?php
							$file = file_get_contents('rest.csv');
							...
							$file = file_get_contents('snacks.csv');
							$items = explode(PHP_EOL, $file);
							foreach($items as $item){
								if ($item ==""){}
								else{
								...
								echo ('<strong>Phone No. :</strong>'.$item[7].'<br>');
								if ($item[4]==" NULL"){
									echo ('<strong>Email :</strong>Not avialable<br>');
								}else{
									echo ('<strong>Email :</strong>'.$item[4].'<br>');
								}
